...
# The 'artifact' stage is a scratch image holding only /imsai-tty-gateway, so --output exports
# just the binary to ./build/imsai-tty-gateway.
...
RUN CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} GOARM=${GOARM} \
    go build -trimpath -ldflags "-s -w" -o /out/imsai-tty-gateway .

# Export-only stage: `docker build --output type=local,dest=...` copies /imsai-tty-gateway out.
FROM scratch AS artifact
COPY --from=build /out/imsai-tty-gateway /imsai-tty-gateway
...
COPY --from=build /out/imsai-tty-gateway /usr/local/bin/imsai-tty-gateway
COPY imsai-gw.toml /etc/imsai-gw.toml
EXPOSE 2323
ENTRYPOINT ["/usr/local/bin/imsai-tty-gateway"]
